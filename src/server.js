...
const { loadCues } = require('./cues');
const { renderVideo } = require('./render');
const { alphaOf } = require('./color');
const { ensureNativeSubtitle } = require('./convertSubtitle');
const { wordsFromText } = require('./subtitles');
const { serializeNative } = require('./subtitles/native');
const {
  loadMarkers, saveMarkers, sanitizeOverrides, backgroundNeedsAlpha,
} = require('./configMarkers');

// Builds the express app for one project (one config.yaml). Pure factory —
// no listen(), no process.exit(): the caller (bin/serve.js for the CLI,
// Electron's main process for the desktop app) owns the HTTP server's
// lifecycle, since Electron needs to tear this down and build a fresh one
// per open/new-project switch rather than run a single instance for the
// process's whole life.
function createServer(configPath) {
  ensureNativeSubtitle(configPath);
  let config = loadConfig(configPath);
  let cues = loadCues(config);
  let markers = loadMarkers(configPath);
  ...
  app.get('/api/data', (req, res) => {
    const publicConfig = {
      font: {
        family: config.font.family, size: config.font.size, weight: config.font.weight, style: config.font.style, url: '/assets/font',
      },
  ...
  app.post('/api/markers', (req, res) => {
    try {
      ...
      saveMarkers(configPath, newMarkers);
      markers = newMarkers;
      res.json({ ok: true });
    } catch (err) {
      res.status(500).json({ ok: false, error: err.message });
  ...
  return {
    app, configPath, projectDir, rendersDir,
  };
}
